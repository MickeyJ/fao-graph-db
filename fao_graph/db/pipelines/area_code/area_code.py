# Generated graph migrator for area_codes
...
class AreaCodeMigrator(GraphMigrationBase):
    ...
    def migrate(self, start_offset: int = 0, mode: str = "create") -> None:
        """Execute the migration for area_codes"""
        logger.info(f"Starting area_codes migration...")
        ...
        try:
            ...
            logger.warning(f"\narea_codes migration interrupted")
            raise
        except Exception as e:
            logger.error(f"Failed to migrate area_codes: {e}")
            raise MigrationError(f"area_codes migration failed: {e}") from e
