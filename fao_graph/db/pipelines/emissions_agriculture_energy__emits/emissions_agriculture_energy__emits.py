...
# Generated migrator for relationship emissions_agriculture_energy__emits
import json
...
class EmissionsAgricultureEnergyEmitsMigrator(GraphMigrationBase):
    """Migrator for EMITS relationships from emissions_agriculture_energy"""
    ...
    def get_migration_query(self) -> str:
        return load_sql("emissions_agriculture_energy__emits.sql", Path(__file__).parent)
    
    def get_verification_query(self) -> str:
        return load_sql("emissions_agriculture_energy__emits_verify.cypher.sql", Path(__file__).parent)

    def get_total_rows_query(self) -> str:
        """Count records matching our filters"""
        return load_sql("emissions_agriculture_energy__emits_total_rows.sql", Path(__file__).parent)
    ...
    def create(self, records, session):
        ...
        for record in records:
            ...
            if self.created % 500 == 0:
                logger.info(f"emissions_agriculture_energy - (area_code_id: {source_id})-[:EMITS]->(item_code_id: {target_id})")
            ...
            query = text(f"""
                SELECT * FROM cypher('fao_graph', $$
                    ...
                    CREATE (source)-[r:EMITS """ + "{" + props_str + "}" + f"""]->(target)
                    RETURN r
                $$) AS (result agtype)
            """)
