...
# Generated migrator for relationship food_balance_sheets__uses
import json
...
class FoodBalanceSheetsUsesMigrator(GraphMigrationBase):
    """Migrator for USES relationships from food_balance_sheets"""
    ...
    def get_migration_query(self) -> str:
        return load_sql("food_balance_sheets__uses.sql", Path(__file__).parent)
    
    def get_verification_query(self) -> str:
        return load_sql("food_balance_sheets__uses_verify.cypher.sql", Path(__file__).parent)

    def get_total_rows_query(self) -> str:
        """Count records matching our filters"""
        return load_sql("food_balance_sheets__uses_total_rows.sql", Path(__file__).parent)
    ...
    def create(self, records, session):
        ...
        for record in records:
            ...
            if self.created % 500 == 0:
                logger.info(f"food_balance_sheets - (area_code_id: {source_id})-[:USES]->(item_code_id: {target_id})")
